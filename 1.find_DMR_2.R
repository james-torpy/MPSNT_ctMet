...
# calculate mean values for selected probes and all tissues:
if (!file.exists(paste0(Robject_dir, "top_10_beta_stats.Rdata"))) {

  all_beta_df <- orig_normal_mtx
  all_beta_df$MPNST <- MPNST_beta_df
  all_beta_df$blood <- blood_mtx

  top_beta_df <- lapply(all_beta_df, function(x) {

  	return <- list(
  	  hyper = x[DMR_top$hyper$probe,],
  	  hypo = x[DMR_top$hypo$probe,]
  	)

  })

  temp_beta_stats <- lapply(top_beta_df, function(x) {
      ...
      lapply(x, function(y) {
        ...
        return(
          t(
              ...
              quantiles <- quantile(z, c(0.1, 0.9), na.rm = TRUE)
              return(
                c(
                  mean = mean(z, na.rm = TRUE), 
                  quart_0.1 = quantiles[1],
                  quart_0.9 = quantiles[2],
                  sd = sd(z, na.rm = TRUE)
             	  )
              )
            })
  ...
  # arrange list to consist of hyper and hypo elements with tissue 
  # elements within:
  for (i in 1:length(temp_beta_stats)) {
  	if (i==1) {
  	  top_beta_stats <- list(
  	  	hyper = list(
  	  	  temp_beta_stats[[i]]$hyper
  	  	),
  	  	hypo = list(
  	  	  temp_beta_stats[[i]]$hypo
  	  	)
  	  )
  	} else {
  	  top_beta_stats$hyper[[i]] = temp_beta_stats[[i]]$hyper
  	  top_beta_stats$hypo[[i]] = temp_beta_stats[[i]]$hypo
  	}
  }
  names(top_beta_stats$hyper) <- names(temp_beta_stats)
  names(top_beta_stats$hypo) <- names(temp_beta_stats)
  
  saveRDS(top_beta_stats, paste0(Robject_dir, "top_10_beta_stats.Rdata"))

} else {

  top_beta_stats <- readRDS(paste0(Robject_dir, "top_10_beta_stats.Rdata"))
...
rm_tissues <- c(
  names(n_vals)[as.numeric(n_vals) < plot_min_sample_no],
  "LUAD_non_malig"
)
plot_beta_stats <- lapply(top_beta_stats, function(x) {
  return(x[!(names(x) %in% rm_tissues)])
})

# update n_vals and names for plot labels:
n_vals <- n_vals[names(n_vals) %in% c(names(plot_beta_stats$hyper), "NF")]

# remove first group of lung samples - include later:
...
# DM probes and plot:
for (i in 1:length(DMR_top)) {

  plot_df <- fetch_DM_beta(
    DMR_obj = DMR_top[[i]],
    beta_stats = plot_beta_stats[[i]],
    probe_coords,
    tissue_key
  )

  if (i==1) {
  	DMR_plot_df <- list(plot_df)
  } else {
  	DMR_plot_df[[i]] <- plot_df
  }

}
names(DMR_plot_df) <- names(DMR_top)
...
for (i in 1:length(DMR_plot_df)) {
  ...
  nm_df <- DMR_plot_df[[i]][
    !(DMR_plot_df[[i]]$tissue %in% c("MPNST", "NF", "blood")),
  ]
  nm_df <- nm_df[order(nm_df$tissue),]

  # reorder so MPNST, NF and blood are top levels
  nm_blood <- rbind(
	DMR_plot_df[[i]][DMR_plot_df[[i]]$tissue == "blood",],
	nm_df
  )
  nm_nf <- rbind(
  ...
  DMR_plot_df[[i]] <- DMR_plot_df[[i]][
    order(DMR_plot_df[[i]]$probe),
  ]
  ...
  print(
    paste0(
      "Plotting candidates, filename = ", plot_dir, "MPNST_", names(DMR_plot_df)[i], "methylated_marker_probes_quantile_bars.png"
    )
  )

  png(
  	paste0(
  	  plot_dir, "top_10_MPNST_", names(DMR_plot_df)[i], 
  	  "methylated_marker_probes_quantile_bars2.png"
  	),
  	res = 300,
  	unit = "in",
  	height = 7,
  ...
  ggsave(
    paste0(plot_dir, "top_10_MPNST_", names(DMR_plot_df)[i], 
    	"methylated_marker_probes_quantile_bars2.pdf"),
    plot = p,
    useDingbats = F,
    width = 10
  )
